...
class ConceptNode:
    ...
    def to_dict(self) -> Dict[str, Any]:
        data = asdict(self)
        data["status"] = self.status.value if isinstance(self.status, ConceptStatus) else str(self.status)
        return data
...
class ConceptEdge:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "source": self.source,
            "target": self.target,
            "relationship": self.relationship.value if isinstance(self.relationship, EdgeRelationship) else str(self.relationship),
            "label": self.label,
        }
