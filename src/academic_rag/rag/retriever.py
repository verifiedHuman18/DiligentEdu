...
def retrieve_student_material_context(
    ...
    try:
        ...
        if not ready_docs:
            if DEBUG_RAG:
                logger.info(f"[DEBUG_RAG] No READY documents found for student '{clean_student_id}' in class {class_filter}")
            return ""
        ...
        if not matches:
            if DEBUG_RAG:
                logger.info(f"[DEBUG_RAG] 0 matches returned from Pinecone student namespace for query='{query}'")
            return ""
        ...
        for match in matches:
            ...
            formatted_chunks.append(f"{chunk_header}\n{text}")
            matched_docs_info.append(f"{mat_name} (p.{page_num}, score={score:.3f})" if score is not None else f"{mat_name} (p.{page_num})")
...
def retrieve_hybrid_academic_context(
    ...
    if has_ncert:
        ...
        combined_parts.append("=== OFFICIAL NCERT TEXTBOOK EXCERPTS ===\n[No direct NCERT textbook excerpt matches found]")
