...
def get_teacher_student_profile(
    ...
    subj_clean = "Mathematics" if "math" in str(subject).lower() else "Science"
    overview = get_teacher_student_overview(student_id, class_level=class_level, subject=subj_clean, db_path=db_path)
    chapters = get_teacher_chapter_statistics(student_id, class_level=class_level, subject=subj_clean, db_path=db_path)
    history = get_teacher_quiz_history(student_id, class_level=class_level, subject=subj_clean, db_path=db_path)
    swat = get_teacher_swat_summary(student_id, class_level=class_level, subject=subj_clean, db_path=db_path)
    status = get_student_status(student_id, class_level=class_level, subject=subj_clean, db_path=db_path)
    action_plan = generate_action_plan(student_id, class_level=class_level, subject=subj_clean, db_path=db_path)
