...
def classify_gemini_error(exc: Exception) -> str:
    ...
    if status_code == 429 or "429" in err_str or "quota" in err_str or "resource_exhausted" in err_str:
...
async def stream_chat_completion(
    ...
    **kwargs: Any,
    ...
    if override_api_key and str(override_api_key).strip():
        ...
        client = AsyncOpenAI(
            base_url=config.gemini_base_url, api_key=str(override_api_key).strip()
        )
