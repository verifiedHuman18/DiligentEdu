...
def render_swat_tab(student_id: str, class_level: Optional[int] = None):
    ...
    if not swat.get("has_data"):
        st.info(
            "No quiz attempts recorded yet for this student ID. Complete a quiz in the "
        ...
        with st.expander(
            ...
            if "questions" in att and att["questions"]:
                for q_idx, q_rec in enumerate(att["questions"], 1):
                    status_text = "[Correct]" if q_rec["is_correct"] else "[Incorrect]"
                    st.markdown(f"**{status_text} Q{q_idx}:** {q_rec['question_text']}")
                    st.caption(
