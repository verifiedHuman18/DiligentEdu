...
def _get_fresh_suggestions(
    ...
    if is_math:
        ncert_pool = list(CLASS_9_MATH_SUGGESTIONS if class_level == 9 else CLASS_10_MATH_SUGGESTIONS)
    else:
...
async def render_tutor_screen(
    ...
    if needs_refresh or stored_suggestions is None or stored_class != class_level or stored_subject != subject:
        stored_suggestions = _get_fresh_suggestions(class_level, subject=subject, student_id=active_student_id)
    ...
    # Chat Input
    prompt_input = st.chat_input(f"Ask any question from NCERT Class {class_level} {subject} or your notes...")
