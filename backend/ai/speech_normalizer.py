...
PHONETIC_REPLACEMENTS: List[Tuple[re.Pattern, str]] = [
    ...
    (
        re.compile(r"\b(flemings?\s*(left|right)\s*hand\s*rule)\b", re.IGNORECASE),
        r"Fleming's \2-hand rule",
    ),
    ...
    (
        re.compile(r"\b(l\s*c\s*m|lowest common multiple|least common multiple)\b", re.IGNORECASE),
        "LCM",
    ),
    (
        re.compile(r"\b(b\s*p\s*t|basic proportionality theorem)\b", re.IGNORECASE),
        "Basic Proportionality Theorem (BPT)",
    ),
    (
        re.compile(r"\b(a\s*p|arithmetic progression)\b", re.IGNORECASE),
...
def normalize_voice_transcript(raw_transcript: str) -> str:
    ...
    if normalized:
        ...
        if (
            ...
            and not first_token.startswith(("x", "y", "z", "a", "b", "c"))
        ):
