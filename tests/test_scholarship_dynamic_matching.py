...
class TestScholarshipDynamicMatching(unittest.TestCase):
    ...
    def test_category_sensitivity(self):
        ...
        self.assertEqual(
            obc_matches["pm-yasasvi-pre-matric"].status, EligibilityStatus.LIKELY_MATCH
        )
        self.assertEqual(
            sc_matches["pm-yasasvi-pre-matric"].status, EligibilityStatus.DOES_NOT_MATCH
        )
    ...
    def test_disability_sensitivity(self):
        ...
        self.assertEqual(
            no_matches["pre-matric-disabilities"].status, EligibilityStatus.DOES_NOT_MATCH
        )
        self.assertEqual(
            yes_matches["pre-matric-disabilities"].status, EligibilityStatus.LIKELY_MATCH
        )
    ...
    def test_school_type_sensitivity(self):
        ...
        self.assertTrue(
            any("matches" in r for r in govt_matches["nmmss"].matched_rules if "school" in r)
        )
        self.assertTrue(
            any(
                "not among eligible types" in u
                for u in pvt_matches["nmmss"].unmatched_rules
                if "school" in u
            )
        )
