...
class TestAIFallback(unittest.TestCase):
    ...
    def test_classify_gemini_error(self):
        ...
        err_429 = Exception("Error code: 429 - {'error': {'message': 'Resource has been exhausted (e.g. check quota)'}}")
        self.assertEqual(classify_gemini_error(err_429), "QUOTA_EXHAUSTED")

        err_401 = Exception("Error code: 401 - API_KEY_INVALID: API key not valid. Please pass a valid API key.")
    ...
    def test_fallback_on_primary_429_quota_exhausted(self, mock_openai_cls):
        ...
        primary_client.chat.completions.create.side_effect = Exception("429 Resource has been exhausted (quota limit reached)")
    ...
    def test_no_fallback_raises_quota_exhausted_error(self, mock_openai_cls):
        ...
        mock_client = MagicMock()
        mock_client.chat.completions.create.side_effect = Exception("429 Resource has been exhausted (quota limit reached)")
        mock_openai_cls.return_value = mock_client
    ...
    def test_invalid_fallback_raises_auth_error(self, mock_openai_cls):
        ...
        primary_client = MagicMock()
        primary_client.chat.completions.create.side_effect = Exception("429 Resource has been exhausted")
    ...
    def test_programming_error_does_not_blindly_fallback(self, mock_openai_cls):
        ...
        primary_client = MagicMock()
        primary_client.chat.completions.create.side_effect = Exception("400 INVALID_ARGUMENT: Bad json schema")
    ...
    def test_streaming_fallback_on_primary_429(self, mock_async_openai_cls):
        ...
        primary_client = MagicMock()
        primary_client.chat.completions.create = AsyncMock(side_effect=Exception("429 Rate limit exceeded"))
